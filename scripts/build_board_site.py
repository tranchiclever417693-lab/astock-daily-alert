...
(function(){
  ...
  if(bt.extras&&bt.extras.length){
    $('#botExtras').innerHTML='<div class="typ"><div class="tname"><span>加分项（非硬性条件）</span></div>'+
      '<ul class="cond">'+bt.extras.map(condLi).join('')+'</ul></div>';
  }
})();
...
(function(){
  ...
  let h=`<b>${bt.label}</b>（收盘后同时满足以下全部条件才触发）：<ul>${li(bt.conds)}</ul>`;
  if(bt.extras&&bt.extras.length){ h+=`<div style="margin:4px 0 2px;color:var(--txt)"><b>加分项</b>（不作硬性条件）：</div><ul>${li(bt.extras)}</ul>`; }
